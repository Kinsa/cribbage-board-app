...
describe('HomeScreen Integration Tests', () => {
  ...
  test('renders correctly', () => {
    const totalScore1 = screen.getByTestId('total-points-value-1');
    const totalScore1Text = screen.getByTestId('total-points-value-1-text');
    const totalScore2 = screen.getByTestId('total-points-value-2');
    const totalScore2Text = screen.getByTestId('total-points-value-2-text');
    ...
    expect(screen.queryByTestId('turn-points-value-1')).not.toBeOnTheScreen();
    expect(screen.queryByTestId('turn-points-value-2')).not.toBeOnTheScreen();
  });
  ...
  test('clicking player 1 button updates total score and renders the turn score for that player', () => {
    ...
    const turnScore1 = screen.getByTestId('turn-points-value-1');
  ...
  test('clicking different players switches turn score', () => {
    ...
    fireEvent.press(player1Button);
    expect(screen.getByTestId('turn-points-value-1')).toBeOnTheScreen();

    // Click player 2 - turn score should still exist but for player 2
    fireEvent.press(player2Button);
    expect(screen.getByTestId('turn-points-value-2')).toBeOnTheScreen();
  });
